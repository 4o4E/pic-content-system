...
- `/`：快捷入口与最近 30 天正式内容每日新增趋势，按北京时间统计。
- `/workspace`：工作台。左侧为组装结果，右侧为 QQ 主动推送或手动上传形成的素材列表；素材列表上方包含搜索、状态过滤和类型过滤。
- `/library`：内容库。展示已入库内容，支持输入筛选 tag、AND / OR 多 tag 过滤、排序、卡片尺寸预设、分页、批量 tag 编辑、批量删除和放回工作台。
- `/tags`：标签管理。用于查找 tag，查看当前使用数量、累计添加次数和累计搜索次数。
- `/events`：接入事件记录。
...
- 左侧导航和顶栏固定显示。
- 工作台搜索放在素材列表过滤区，不放顶栏。
- tag 输入都应支持输入筛选和下拉选择；是否允许直接创建新 tag 由具体业务场景决定。
- tag 建议按累计添加与搜索次数优先显示；新增内容标签时累计添加次数，明确选择标签进行搜索时累计搜索次数。
- 图片预览中只有高度超过宽度四倍的图片从顶部进入长图阅读；其他图片完整显示。横向滚轮切图，右键或工具栏可复制图片；移动端下方菜单可切换图集，长按图片可打开复制、下载和分享菜单。
...
- 内容库分页在桌面端保留紧凑分页和每页数量选择；移动端隐藏总条数，并用单个“每页条数”按钮循环切换页大小，避免控件换行。
